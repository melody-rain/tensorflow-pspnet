            ...
            for i, unit in enumerate(block.args):
                with tf.variable_scope('unit_%d' % (i + 1), values=[net]):
                    ...
                    net = block.unit_fn(net,
                                        ...
                                        stride=unit_stride,
                                        rate=unit_rate)
                    if tf.contrib.framework.get_name_scope().endswith('pspnet_v1_101/block3/unit_2'):
                        aux_net = net
                        aux_net = slim.utils.collect_named_outputs(outputs_collections, sc.name, aux_net)
